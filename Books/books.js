...
function addBookCard(bookNumber) {
  ...
  const htmlBook = `
        <h3 class="bookTitle">${capitalizeFirstLetter(
          myLibrary[bookNumber].title
        )}</h3>
        <p class="bookAuthor">${capitalizeFirstLetter(
          myLibrary[bookNumber].author
        )}</p>
        <p class="bookPages">${myLibrary[bookNumber].pages} Pages</p>
        <button id="read${bookNumber}" class="read-btn ${
  ...
  document
    .getElementById(`bookCard${bookNumber}`)
    .insertAdjacentHTML("afterbegin", htmlBook);
}
